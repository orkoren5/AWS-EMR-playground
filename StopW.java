...
public class StopW {
	 ...
	 * 
	 * @param wordForCk
	 * @return if (wordForCk is StopWord)
	 */
	protected boolean isContains(String wordForCk)
	{
		return stopWords.contains(wordForCk);
	}
	
	/**
	 * 1.Lower Case
	 * 2.remove "s'" and '
	 * 3.only words contains a-zA-Z0-9
	 * 4. ck if stopWord
	 * 5. real word, not space or same ascii wired char
	 * @param word 
	 * @return isStopWord?
	 */
	public boolean isStopWord(String word){
		...
		//3
		//word = word.replaceAll("[^a-zA-Z0-9']",""); //same as \\w
		word = word.replaceAll("[^\\w]", "");
		//4
		if (isContains(word))
			word = "";
		//5
		else if ((word.length() < 2) || !Character.isLetter(word.charAt(0)))
					word = "";
